...
#' 
#' @export
andersen.gill.score <- function(x, null.model, beg = 1, end = ncol(x)) {
  ...
  if(null.model@float) 
    t <- GWAS_score_test_AG_bed_float(x@bed, x@p, times, beg - 1L, end - 1L, null.model@ptr)
  else
    t <- GWAS_score_test_AG_bed_double(x@bed, x@p, times, beg - 1L, end - 1L, null.model@ptr)
